...
const PopularChannels: React.FC<Props> = ({ open }) => {
    ...
    <List>
        {dataCRUD.map((item) => {
            return <ListItem key={item.id} disablePadding sx={{ display: "block" }} dense={true}>
                        ...
                        <ListItemText
                            primary={
                                <Typography variant="body2"
                                    sx={{fontWeight: 700, lineHeight: 1.2, textOverflow: "ellipsis",
                                    overflow: "hidden", whiteSpace: "nowrap"}} >{item.name}</Typography>
                            }
                            secondary={
                                <Typography variant="body2" sx={{fontWeight: 500, lineHeight: 1.2,
                                color: "textSecondary",}}>{item.category}</Typography>
                            }
                            sx={{ opacity: open ? 1 : 0 }} 
